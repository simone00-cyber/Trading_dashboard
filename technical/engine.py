...
def _record(name: str, category: str, direction: str, anchors: list[tuple[Any, float]], *, frame: pd.DataFrame,
            ...
            highlight_end: Any | None = None, notes: str = "") -> dict[str, Any]:
    ...
    return {
        ...
        "category": category,
        "direction": direction,
        "confidence": int(round(np.clip(confidence, 0, 100))),
...
def detect_pattern_details(frame: pd.DataFrame, settings: TechnicalSettings) -> list[dict[str, Any]]:
    """Transparent heuristic recognition. Candidates require visual review and are never trading signals."""
    ...
    for points, label, direction, category in ((highs, "top", "BEARISH", "REVERSAL"), (lows, "bottom", "BULLISH", "REVERSAL")):
        if len(points) >= 2:
            ...
            if similarity >= 1 - tol:
                ...
                trigger = float(between["Low"].min() if label == "top" else between["High"].max())
                details.append(_record(f"Potential double {label}", category, direction, list(zip(pts.index, pts.astype(float))), frame=recent,
                                       confidence=55 + similarity * 35, trigger=trigger, notes="Two comparable swing extremes."))
        if len(points) >= 3:
            ...
            if dispersion <= tol:
                ...
                trigger = float(span["Low"].min() if label == "top" else span["High"].max())
                details.append(_record(f"Potential triple {label}", category, direction, list(zip(pts.index, vals)), frame=recent,
                                       confidence=65 + max(0, 1 - dispersion / max(tol, 1e-9)) * 25, trigger=trigger,
                                       notes="Three comparable swing extremes."))
    ...
    if len(highs) >= 3:
        ...
        if shoulder_similarity >= 1 - tol * 1.6 and prominence >= max(0.02, tol * 0.7):
            ...
            details.append(_record("Potential head and shoulders", "REVERSAL", "BEARISH", anchors, frame=recent,
                                   ...
                                   lower=[(between1, n1), (between2, n2)], notes="Head above two comparable shoulders; neckline shown."))
    if len(lows) >= 3:
        ...
        if shoulder_similarity >= 1 - tol * 1.6 and prominence >= max(0.02, tol * 0.7):
            ...
            details.append(_record("Potential inverse head and shoulders", "REVERSAL", "BULLISH", anchors, frame=recent,
                                   confidence=55 + shoulder_similarity * 25 + min(prominence / 0.15, 1) * 15, trigger=neckline,
                                   upper=[(between1, n1), (between2, n2)], notes="Head below two comparable shoulders; neckline shown."))

    # Triangles, wedges, channels, rectangles and broadening formations.
    if len(highs) >= 3 and len(lows) >= 3:
        ...
        if abs(hs) <= flat and ls > flat:
            details.append(_record("Potential ascending triangle", "CONTINUATION", "BULLISH", hpts + lpts, frame=recent, confidence=78, trigger=float(hp.mean()), upper=hpts, lower=lpts))
        elif hs < -flat and abs(ls) <= flat:
            details.append(_record("Potential descending triangle", "CONTINUATION", "BEARISH", hpts + lpts, frame=recent, confidence=78, trigger=float(lp.mean()), upper=hpts, lower=lpts))
        elif hs < -flat and ls > flat and narrowing:
            details.append(_record("Potential symmetrical triangle / pennant", "COMPRESSION", "NEUTRAL", hpts + lpts, frame=recent, confidence=74, upper=hpts, lower=lpts))
        elif hs > flat and ls > flat and narrowing:
            details.append(_record("Potential rising wedge", "COMPRESSION", "BEARISH", hpts + lpts, frame=recent, confidence=72, trigger=float(lp.iloc[-1]), upper=hpts, lower=lpts))
        elif hs < -flat and ls < -flat and narrowing:
            details.append(_record("Potential falling wedge", "COMPRESSION", "BULLISH", hpts + lpts, frame=recent, confidence=72, trigger=float(hp.iloc[-1]), upper=hpts, lower=lpts))
        elif hs > flat and ls < -flat:
            details.append(_record("Potential broadening formation", "VOLATILITY", "NEUTRAL", hpts + lpts, frame=recent, confidence=70, upper=hpts, lower=lpts))
        elif abs(hs) <= flat and abs(ls) <= flat:
            details.append(_record("Potential rectangle", "CONSOLIDATION", "NEUTRAL", hpts + lpts, frame=recent, confidence=76, upper=hpts, lower=lpts))
        elif hs > flat and ls > flat and abs(hs - ls) <= max(abs(hs), abs(ls)) * 0.45:
            details.append(_record("Potential ascending channel", "TREND", "BULLISH", hpts + lpts, frame=recent, confidence=68, upper=hpts, lower=lpts))
        elif hs < -flat and ls < -flat and abs(hs - ls) <= max(abs(hs), abs(ls)) * 0.45:
            details.append(_record("Potential descending channel", "TREND", "BEARISH", hpts + lpts, frame=recent, confidence=68, upper=hpts, lower=lpts))
    ...
    if len(recent) >= 80:
        ...
        if fit > 0.55 and abs(coef[0]) > window.mean() * 0.02:
            ...
            details.append(_record(label, "REVERSAL", direction, anchors, frame=recent, confidence=55 + fit * 35))

    # Keep strongest candidate of each name and sort by confidence.
    unique: dict[str, dict[str, Any]] = {}
    for item in details:
        if item["name"] not in unique or item["confidence"] > unique[item["name"]]["confidence"]:
            unique[item["name"]] = item
    return sorted(unique.values(), key=lambda x: (-x["confidence"], x["name"]))
...
def analyse_technical(ticker: str, frame: pd.DataFrame, settings: TechnicalSettings) -> TechnicalSnapshot:
    ...
    if support:
        ...
        if _confirmed_break(close, support["low"], "below", settings.breakout_buffer_pct, settings.breakout_confirmations): setups.append("Support breakdown")
    if resistance:
        ...
        if _confirmed_break(close, resistance["high"], "above", settings.breakout_buffer_pct, settings.breakout_confirmations): setups.append("Resistance breakout")
